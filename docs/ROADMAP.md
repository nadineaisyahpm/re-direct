...
| **Phase 6E-B** | Proxy `POST /v1/trail` endpoint: handler, Zod-strict validation, denylist extension, provider adapter for trail prompts, response normalization, tests. Proxy repo only, no iOS code. | proposed, depends on 6E-A |
| **Phase 6E-C** | iOS DTOs + HTTP client: `TrailRequestPayload` (privacy-safe value type), `TrailResponse` + `TrailStep` Codables, `AIProxyHTTPClient.callTrail(_:)`. Tests for encode/decode + allowlist + forbidden-fields-absent. No UI. | proposed, depends on 6E-B |
| **Phase 6E-D** | iOS UI: `deepen` affordance on `LooseEndRow`, `TrailSheet` (transient response + step list), accept/cancel, `TrailMaterializer` implementing the §6 bridge. | proposed, depends on 6E-C |
| **Phase 6E-E** | Seeded `TopicTrail` fallback when proxy unavailable. May require a seed-content audit. | proposed, depends on 6E-D |
| **Phase 6E-F** | Deferred follow-on triggers (from Daily Direct card; from existing thread "extend with AI"). Revisit after 6E-D ships and trail quality is validated. | deferred |
